...
- Focused app commands:
  - `pnpm --filter regarde.inspector dev` starts the inspector via Portless on `https://regarde.inspector.localhost:1355` and also starts `agentation-mcp server`.
  - `pnpm --filter regarde.inspector dev:vite` starts the inspector directly through Vite and also starts `agentation-mcp server`.
  - `pnpm --filter regarde.inspector portless:proxy` starts the unprivileged HTTPS Portless proxy on port `1355`.
