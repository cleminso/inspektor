...
The entry exports `BrandSiteFrame`, `BrandHero`, `BrandProductPreview`, `BrandButtonLink`, `BrandTextLink`, and `BrandWordmark`. Public props exclude consumer styling and content escape hatches.
...
`BrandButtonLink` is the primary action treatment. `BrandTextLink` is the neutral secondary and external-link treatment; its optional icon-only mode preserves the minimum interaction target.
