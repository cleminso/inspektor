...
This playbook defines when and how to defer optional runtime dependencies.
...
Deferred imports must reduce initial work without weakening React ownership or public behavior.

### Keep the static graph minimal

The static module graph contains only dependencies needed for the first usable render.
...
Deferred behavior stays inside the React components and providers that own it.
...
A deferred interaction keeps its controlled API and usable static state while loading.
...
Keep deferred loading local until several features share the same lifecycle and errors.
...
Import-boundary analysis starts with the package export and module graph that each consumer actually loads.
...
The pattern separates static rendering, deferred behavior, and a dependency-free bridge.

### Static module responsibilities

The static module owns public contracts, usable markup, loading policy, and transition state.
...
The deferred module contains every runtime dependency and behavior that needs its provider.
...
The static bridge exposes narrow React slots without importing the deferred dependency.
...
One loading policy and one shared retryable promise govern each deferred module.
...
Tests and builds verify the static state, deferred behavior, lifecycle transition, and emitted chunks.

### Required behavioral tests

Behavioral tests cover loading boundaries, ref ownership, controlled output, and focus restoration.
...
These focused checks verify behavior, types, lint, and emitted application chunks.
...
Update the corresponding `todo/{elementName}.md` checklist with the settled implementation decision and validation coverage.

## Patterns to avoid

These patterns break module boundaries, React ownership, lifecycle clarity, or performance claims.
...
Use this checklist to review a deferred import boundary before merging it.
