...
This document defines the accepted scope, boundaries, batches, and checks for applying semantic tokens across active `@inspektor/ds` components.

## Table of contents

The sections cover the objective, commands, repository ownership, coding and testing rules, implementation batches, and accepted decisions.
...
These rules distinguish required implementation choices from changes that need approval and prohibited escape hatches.
...
The work is grouped by component role so related state and token decisions stay consistent.

### Batch 1: field controls

This batch normalizes form-control state and compound field semantics.
...
This batch normalizes action, toggle, selection, loading, and disabled states.
...
This batch maps notification and overlay intent while normalizing interactive layout states.
...
This batch constrains foundational layout and text APIs while completing token coverage.
...
The work succeeds when components, public APIs, documentation, tests, and builds agree on the semantic model.
...
These decisions define the accepted API restrictions, token roles, value mappings, and layer order.
...
Component implementation follows [[lat.md/create-components#Creating components]], while wrapper ownership follows [[lat.md/baseUiComponentAnatomy#Anatomy of Base UI components#Base UI versus Inspektor ownership]].
