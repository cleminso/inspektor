...
The Live queries view presents sampled server subscription telemetry as a swimlane timeline with table lanes, query tracks, and snapshot details.

## Table of contents

This page defines the product structure, interactions, states, and scope for the Live queries view.
...
Its telemetry contract and limits are defined in [[lat.md/query-subscriptions#Jazz live-query and subscription telemetry]].
...
The timeline maps successful and failed captures to present, absent, and unknown intervals.
...
A failed request does not prove that a group was absent. If failed attempts are represented in the timeline, the entire interval uses an unknown-state treatment.

The time header remains a time label. It must not be replaced with the word "Unavailable." The failure message belongs in the status UI or interval tooltip.
...
The initial interface must fetch telemetry, retain bounded history, render table lanes, and expose selected query metadata.
...
The interface does not infer data or timing that the standalone telemetry endpoint does not expose.
