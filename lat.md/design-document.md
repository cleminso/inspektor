# Inspektor design document

This document defines the product direction, architecture, interaction model, and operational constraints for Inspektor.

## Metadata

This metadata identifies the document, its status, and its audience.

Author: Cleminso
Status: v1 - Working progress
Repo-URL: https://github.com/cleminso/inspektor/
Doc-URL: https://github.com/cleminso/inspektor/blob/main/lat.md/design-document.md
...
This diagram maps application startup, runtime ownership, routes, table exploration, and Jazz server access.
...
Inspektor is a schema-driven admin client that connects directly to the Jazz sync system.
...
Five subsystems compose the Inspektor application.
...
The focused routing decisions and examples live in [[routing#Inspektor route structure|Inspektor route structure]].
...
Connection management deliberately excludes account and Jazz Cloud project management.
...
These user contexts explain the needs that shape the feature.
...
These capabilities define the required feature scope.
...
The first-run flow validates credentials, selects a schema, and opens the table route.
...
Runtime bootstrap turns a resolved connection context into an active Jazz admin client.
...
Each startup failure has one owner and a defined effect on the active session.
...
The data table is the Table Explorer's read-first surface for rows, selection, navigation, and edits.

Detailed row selection, cell selection, side-pane, column-selection, and bulk-edit behavior lives in the focused
[[tableExplorerBehaviors#Table Explorer selection and pane behavior|Table Explorer selection and pane behavior specification]]. This document retains the product and
...
Schema metadata determines each column's compact type marker and value treatment.
...
| Integer                        | Start-aligned whole number.                                                                         | Numeric text input with integer parsing and validation.                                                          |
| Float                          | Start-aligned readable number; preserve full precision outside the compact preview.                 | Numeric text input that preserves intermediate editing states and validates finite values.                       |
...
Checkboxes select rows for the complete-row pane, while cell focus and editing remain independent.
...
Inline editors and the complete-row pane share one mutation model and Apply boundary.
...
The first version excludes mutation and query operations that lack safe generic semantics.
...
Live queries telemetry presents server-visible subscription snapshots and links supported queries to tables.
...
Live queries only shows server-visible subscription snapshots. Local-only queries that never reach server telemetry, short-lived reads, wrong connection context, or telemetry failure can all explain an empty view. See [[query-subscriptions#Jazz live-query and subscription telemetry|Jazz live-query and subscription telemetry]].
...
These boundaries keep the feature focused on its stated product purpose.
...
The Table Schema item presents stored metadata but does not edit or redesign schemas.
...
These scenarios describe the main end-to-end product flows.
...
Runtime bootstrap performance work is covered by the following performance constraints.

## Performances

These constraints keep startup and deferred interactions out of the initial application graph where possible.
