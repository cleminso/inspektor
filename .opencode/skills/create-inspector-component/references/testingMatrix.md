...
1. Identify the Inspector-owned runtime behavior, type contract, architectural invariant, or wrapper boundary the test protects.
2. If no such Inspector contract exists, delete the test.
...
4. If Inspector only assembles upstream parts, keep one minimal semantic and accessibility smoke test that can detect disconnected wiring.
5. If Inspector changes an upstream default, transforms a prop, composes handlers or refs, derives state, owns lifecycle behavior, constrains an API, or establishes an architectural policy, test that exact difference.
...
Before retaining a test, identify the responsible Inspector source lines and state the regression it protects. A test name must describe that regression rather than an upstream primitive's generic behavior.
