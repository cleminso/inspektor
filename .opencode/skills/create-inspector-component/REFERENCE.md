...
Inspektor wrappers are behaviorally transparent to Base UI by default and visually closed. Base UI owns interaction, accessibility behavior, state, and generated DOM contracts. Inspektor owns presentation and any additional semantic design decisions.
...
3. Type derivation: whether its public type comes from Base UI, React, or Inspektor.
4. Transformation: whether Inspektor renames, combines, constrains, consumes, or injects it.

Classify each difference as a preserved Base UI contract, intentional Inspektor constraint, Inspektor extension, accidental divergence, or per-component decision.
...
| Expose `className`                                    | No     | Inspektor is the presentation source of truth                           |
...
| Introduce Inspektor props speculatively               | No     | Each extension needs a concrete system decision                         |
...
- Base UI props fixed internally by Inspektor
- Base UI props transformed or replaced by Inspektor
- names that collide with Inspektor-owned semantic props
...
- [Public API policy](references/publicApiPolicy.md): inheritance, omissions, transformations, and Inspektor props.
...
- Inspektor props have a requirement, constrained type, literal default, and mapping
