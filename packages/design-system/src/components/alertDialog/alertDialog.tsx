...
export type AlertDialogActionsProps = Omit<
...
function AlertDialogRoot({ defaultOpen = false, ...props }: AlertDialogRootProps) {
  return <BaseAlertDialog.Root {...props} defaultOpen={defaultOpen} />;
}
...
const AlertDialogContent = forwardRef<HTMLDivElement, AlertDialogContentProps>(
  function AlertDialogContent({ keepMounted = false, ...props }, ref) {
    const backdropStateStyles = createStateStyleProps<BaseAlertDialog.Backdrop.State>(
      (state) => [
    ...
    const viewportStateStyles = createStateStyleProps<BaseAlertDialog.Viewport.State>(
      (state) => [
    ...
    return (
      <BaseAlertDialog.Portal keepMounted={keepMounted}>
        ...
        <BaseAlertDialog.Viewport {...viewportStateStyles}>
          <BaseAlertDialog.Popup {...props} ref={ref} {...popupStateStyles} />
...
const AlertDialogActions = forwardRef<HTMLDivElement, AlertDialogActionsProps>(
  function AlertDialogActions(props, ref) {
    return <div {...props} ref={ref} {...stylex.props(alertDialogStyles.actions)} />;
  },
);

const AlertDialogClose = forwardRef<HTMLElement, AlertDialogCloseProps>(
  function AlertDialogClose({ render = <Button variant="secondary" />, ...props }, ref) {
    const stateStyleProps = createStateStyleProps<BaseAlertDialog.Close.State>((state) => [
      state.disabled === true
        ? alertDialogStyles.closeDisabled
        : alertDialogStyles.closeEnabled,
    ]);
