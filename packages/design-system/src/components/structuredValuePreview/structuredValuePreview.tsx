...
function createPreviewParts(model: StructuredValuePreviewModel): StructuredValuePreviewParts {
  ...
  if (model.kind === 'array') {
    ...
    return {
      content: [...model.entries, ...(model.continuation === 'truncated' ? ['…'] : [])].join(', '),
...
export function StructuredValuePreview({ model, variant = 'json' }: StructuredValuePreviewProps) {
  ...
  return (
    <span
      translate="no"
      {...stylex.props(structuredValuePreviewStyles.root)}
    >
      {variant === 'typedJson' ? (
        <span
      ...
      >
        {preview.marker === null ? null : (
          <span {...stylex.props(structuredValuePreviewStyles.markerRail)}>{preview.marker}</span>
        )}
        {preview.content === null ? null : (
          <span {...stylex.props(structuredValuePreviewStyles.preview)}>{preview.content}</span>
        )}
      </code>
    </span>
  )
