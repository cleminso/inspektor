...
describe("Command", () => {
  ...
  it("uses substring matching for non-Latin text", () => {
    ...
    render(
      <Command.Root items={localizedItems} itemToStringLabel={(item) => item.label}>
        ...
        <Command.List>
          {localizedItems.map((item) => <Command.Item key={item.label} value={item}>{item.label}</Command.Item>)}
        </Command.List>
      </Command.Root>,
    );
  ...
  it("does not select before explicit keyboard navigation when auto highlighting is disabled", () => {
    ...
    render(
      <Command.Root autoHighlight={false} items={items} itemToStringLabel={(item) => item.label}>
        ...
        <Command.List>
          {items.map((item) => (
            <Command.Item key={item.label} value={item} onClick={() => onSelect(item.label)}>{item.label}</Command.Item>
          ))}
        </Command.List>
      </Command.Root>,
    );
      ...
      <Command.Root items={operatorItems} itemToStringLabel={(item) => item.label}>
        ...
        <Command.List>
          {operatorItems.map((item) => (
            <Command.Item key={item.label} value={item}>{item.label}</Command.Item>
          ))}
        </Command.List>
      </Command.Root>,
    );
      ...
      <Command.Root items={items} itemToStringLabel={(item) => item.label}>
        ...
        <Command.List>
          <Command.Item value={items[0]}>
            <Command.ItemText data-testid="inline-item" layout="inline" label="id" description="UUID" />
          </Command.Item>
        </Command.List>
      </Command.Root>,
    );
  ...
  it("forwards item refs", () => {
    ...
    render(
      <Command.Root items={items} itemToStringLabel={(item) => item.label}>
        <Command.Input aria-label="Filter columns" />
        <Command.List>
          <Command.Item ref={ref} value={items[0]}>{items[0].label}</Command.Item>
        </Command.List>
      </Command.Root>,
    );
  ...
  it("provides a close action and restores trigger focus", async () => {
    function Example() {
      ...
      return (
        <>
          <button type="button" onClick={() => setOpen(true)}>Add filter</button>
