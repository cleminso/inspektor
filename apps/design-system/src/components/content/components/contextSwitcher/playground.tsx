...
export function ContextSwitcherPlayground({ children }: { children?: ReactNode }): ReactElement {
  ...
  const preview = (
    <ContextSwitcher.Root items={branches} defaultValue="main" disabled={state.disabled}>
      <ContextSwitcher.Trigger label="Switch branch" size={state.size} width={state.triggerWidth}>
        <ContextSwitcher.Value placeholder="Select branch" />
      </ContextSwitcher.Trigger>
