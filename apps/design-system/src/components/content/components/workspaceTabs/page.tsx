...
export function WorkspaceTabsPage(): ReactElement {
  return (
    <WorkspaceTabsPlayground>
      <Section
        title="Behavior"
        description="Workspace Tabs composes fixed leading and trailing control areas around a horizontally scrollable tab list. Tabs retain their full title width, pass beneath the opaque fixed areas while scrolling, and reveal the selected tab at the nearest edge. A controlled value list and reorder callback enable horizontal pointer dragging; a successful drop activates the dragged tab, while a canceled drag preserves the active view. Tab moves sequentially through each tab and close action without activating a view; Enter opens reorder actions below the focused tab, arrow keys navigate between tabs, Shift plus Arrow reorders directly, and Delete closes the focused view. Closable items compose the tab and close action as adjacent controls with stable spacing, so revealing the close action does not change tab width."
      />
