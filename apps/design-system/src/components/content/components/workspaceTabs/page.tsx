...
export function WorkspaceTabsPage(): ReactElement {
  return (
    <WorkspaceTabsPlayground>
      <Section
        title="Behavior"
        description="Workspace Tabs composes fixed leading and trailing control areas around a horizontally scrollable tab list. Tabs retain their full title width, pass beneath the opaque fixed areas while scrolling, and reveal the selected tab at the nearest edge. Replaceable tabs use an italic title treatment; applications own replacement and persistence state. Consumers can handle tab-button double clicks and contribute Context Menu parts, which share one menu with built-in reorder actions. Tab moves sequentially through each tab and close action without activating a view; Enter opens available actions below the focused tab, arrow keys navigate between tabs, Shift plus Arrow reorders directly, and Delete closes the focused view. A controlled value list and reorder callback enable horizontal pointer dragging; a successful drop activates the dragged tab, while a canceled drag preserves the active view. Closable items keep the complete title and close action mounted as sibling controls."
      />
