...
function ControlRow<TState extends PlaygroundState>({
  ...
  return (
    <Box alignItems="center" gap="l">
      <Box flex={1} flexBasis={0} minWidth={0}>
        <Text as="label" htmlFor={controlId} variant="body" color="muted">
          {control.label}
        </Text>
      </Box>
      <Box flex={1} flexBasis={0} minWidth={0} justifyContent="end">
  ...
  return (
    <Box flexDirection="column" gap="xl">
      <Box alignItems="center" justifyContent="between" gap="l">
        ...
        <Button variant="ghost" size="s" onClick={onReset} aria-label="Reset controls">
          Reset
        </Button>
      </Box>
