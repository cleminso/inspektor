...
const componentEntries = [
  ...
  ...['Root', 'Item', 'SelectionControl', 'Trigger', 'Action'].map((part) => ({
    ...
    part,
    inheritedProps: part === 'Root' ? ['children', 'aria-label', 'aria-labelledby'] : [],
  })),
  ...
  ...['Root', 'Content', 'Details', 'Summary', 'Actions'].map((part) => ({
    ...
    part,
    inheritedProps: part === 'Root' ? ['aria-label', 'aria-labelledby', 'children'] : ['children'],
  })),
  ...[
    'Root',
    'Viewport',
  ...
  ...['Root', 'Label', 'Trigger', 'Content', 'Item'].map((part) => ({
...
function extractComponentProps(project, entry) {
  ...
  return propsType
    ...
    .flatMap((symbol) => {
      ...
      if (runtimeDefaults.has(symbol.getName())) {
        prop.defaultValue = runtimeDefaults.get(symbol.getName())
      }
      prop.description = getDescription(symbol, typeChecker)
