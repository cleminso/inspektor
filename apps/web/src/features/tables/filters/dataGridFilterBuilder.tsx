...
function getVisibleColumns(columns: readonly ColumnDescriptor[]): ColumnDescriptor[] {
  return [tableIdFilterColumn, ...columns].filter((column) =>
    visibleColumnTypes.has(column.column_type.type),
  )
}
  ...
  for (const operator of getFilterOperatorsForColumn(column)) {
    ...
    if (operator === 'isNull') {
      options.push(
        {
          label: 'Is null',
          keywords: operatorKeywords.isNull,
          operator,
          code: operator,
          nullValue: true,
        },
...
const timestampComparisonOperators = new Set<TableFilterOperator>([
  'eq',
  'ne',
  'gt',
  'gte',
...
function formatDateValue(value: unknown): string {
  ...
  const date =
    Number.isFinite(numericValue) === true ? new Date(numericValue) : new Date(String(value))
  ...
  if (
    ...
    if (['gt', 'gte', 'lt', 'lte'].includes(operator)) {
      options.push(
        ...
        {
          label: '30 days ago',
          keywords: ['month', 'thirty days'],
          value: String(getStartOfDay(-30)),
        },
      )
    }
    options.push({
...
function getClauseIssue(
  ...
  try {
    if (clause.operator === 'in') {
      ...
      parseFilterTokens(
        column,
        clause.value.map((value) => String(value)),
      )
...
export function DataGridFilterBuilder({
  ...
  function stageDraft(nextDraft: FilterDraft) {
    ...
    setStagedDrafts((current) => [...current, { key: crypto.randomUUID(), draft: nextDraft }])
  ...
  async function removeFilter(filter: TableFilterClause, fallbackIndex: number) {
    ...
    const matchingIndex =
      filters[fallbackIndex]?.id === filter.id
        ? fallbackIndex
    ...
    pendingRemovalFocusRef.current = {
      ...
      targetKey:
        nextTargetIndex < 0 ? null : (createFilterRenderKeys(nextFilters)[nextTargetIndex] ?? null),
    }
    try {
      setIsUpdatingFilters(true)
      setUpdateIssue(null)
  ...
  const operatorOptions =
    draft?.column === null || draft?.column === undefined ? [] : getOperatorOptions(draft.column)
          ...
          <>
            ...
            >
              {filters.map((filter, index) => {
                ...
                const editStage =
                  clauseIssue === 'Column no longer exists.'
                    ? 'column'
                    : clauseIssue === 'Operator is no longer supported.' ||
                        filter.operator === 'isNull'
                      ? 'operator'
                      : 'value'
                return (
                  <DataGridFilterClause.Root
                    ref={(node) => {
                      clauseRootRefs.current[index] = node
                    }}
                  ...
                  >
                    <DataGridFilterClause.Trigger
                      ref={(node) => {
                        clauseTriggerRefs.current[index] = node
                      }}
      ...
      >
        ...
        {draft === null ? null : (
          ...
          >
            <Command.InputRow aria-label="Filter draft">
              {stagedDrafts.map((stagedDraft) => {
                ...
                const stagedOperatorLabel = isNullDraft
                  ? stagedDraft.draft.rawValue === 'true'
                    ? 'is null'
                    : 'is not null'
                ...
                return (
                  <DataGridFilterClause.Root key={stagedDraft.key}>
                    ...
                    >
                      ...
                      {isNullDraft ? null : (
                        <DataGridFilterClause.Value>{stagedValue}</DataGridFilterClause.Value>
                      )}
                    </DataGridFilterClause.Trigger>
                    <DataGridFilterClause.Remove
            ...
            {draft.stage !== 'value' ? null : (
              <Command.Input
                ...
                onKeyDown={(event) => {
                  if (
                    ...
                    if (draft.operator === 'in' && draft.rawValue.trim().length > 0) {
                      const nextDraft = setFilterDraftTokens(draft, [
                        ...draft.tokens,
                        draft.rawValue.trim(),
                      ])
                ...
                onPaste={(event) => {
                  ...
                  const pastedTokens = tokenizePastedFilterValues(
            ...
            {draft.stage === 'date' ||
            (draft.stage === 'value' && stageItems.length === 0) ? null : (
              <Command.List>
                ...
                <Command.Group>
                  ...
                  {stageItems.map((item) => (
                    ...
                    >
                      <Command.ItemText
                        data-testid={
                          'column' in item ? `command-item-text-${item.column.name}` : undefined
                        }
                        label={item.label}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      updateValue(
                        setFilterDraftTokens(
                          draft,
                          draft.tokens.filter((_, tokenIndex) => tokenIndex !== index),
                        ),
                      )
                    }
                  >
