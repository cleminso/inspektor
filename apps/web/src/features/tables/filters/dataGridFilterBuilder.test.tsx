...
const columns = [
  ...
  { name: "status", column_type: { type: "Enum", variants: ["active", "paused"] }, nullable: false },
...
describe("DataGridFilterBuilder", () => {
  it("edits one compound clause atomically and preserves the clause ID", async () => {
    ...
    const filters: TableFilterClause[] = [{ id: "filter-1", column: "name", operator: "eq", value: "Ada" }];
    render(<DataGridFilterBuilder columns={columns} filters={filters} onFiltersChange={onFiltersChange} />);
  ...
  it("keeps a stale clause removable and repairs its column in place", async () => {
    const onFiltersChange = vi.fn();
    const filters: TableFilterClause[] = [{ id: "stale", column: "removed", operator: "eq", value: "Ada" }];
    render(<DataGridFilterBuilder columns={columns} filters={filters} onFiltersChange={onFiltersChange} />);
  ...
  it("focuses the last applied clause before removing clauses from the end", async () => {
    function ControlledExample() {
      ...
      return <DataGridFilterBuilder columns={columns} filters={filters} onFiltersChange={setFilters} />;
    }
  ...
  it("restores clause focus after an asynchronous removal is rendered", async () => {
    ...
    fireEvent.keyDown(screen.getByRole("group", { name: "Filter age is greater than 20" }), { key: "Backspace" });

    await act(async () => vi.advanceTimersByTimeAsync(10));

    expect(document.activeElement).toBe(screen.getByRole("group", { name: "Filter name equals Ada" }));
  });
  ...
  it("lists distinct loaded values while keeping free-form value entry available", async () => {
    ...
    render(
      <DataGridFilterBuilder
        columns={columns}
        filters={[]}
        rows={[{ id: "1", name: "Ada" }, { id: "2", name: "Grace" }, { id: "3", name: "Ada" }]}
    ...
    expect(onFiltersChange).not.toHaveBeenCalled();
    expect(await screen.findByRole("button", { name: "Edit draft filter name equals Grace" })).toBeTruthy();
  ...
  it("keeps completed clauses staged until Enter applies the command", async () => {
    ...
    expect(onFiltersChange).not.toHaveBeenCalled();
    expect(await screen.findByRole("button", { name: "Edit draft filter name equals Ada" })).toBeTruthy();
  ...
  it("keeps the command open after removing its only staged clause", async () => {
    ...
    const removeDraft = await screen.findByRole("button", { name: "Remove draft filter name equals Ada" });
    removeDraft.focus();
  ...
  it("formats applied timestamp values as year-month-day", () => {
    render(
      <DataGridFilterBuilder
        columns={columns}
        filters={[{ id: "created", column: "createdAt", operator: "eq", value: new Date(2026, 7, 19).getTime() }]}
        onFiltersChange={vi.fn()}
      />,
    );

    expect(screen.getByRole("button", { name: "Edit filter createdAt equals 2026-08-19" }).textContent).toContain("2026-08-19");
  });
