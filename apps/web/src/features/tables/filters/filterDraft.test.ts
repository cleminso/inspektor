...
describe('filter draft transitions', () => {
  ...
  it('enters editing at the activated stage and atomically preserves clause order', () => {
    ...
    const draft = setFilterDraftValue(editFilterDraft(filters[0], 0, 'value', nameColumn), 'Grace')

    expect(applyFilterDraft(draft, filters)).toEqual({
      filters: [{ id: 'first', column: 'name', operator: 'eq', value: 'Grace' }, filters[1]],
    })
  })

  it('follows a unique clause ID when another filter shifts its original index', () => {
    const original: TableFilterClause = {
      id: 'second',
      column: 'name',
      operator: 'eq',
      value: 'Ada',
  ...
  it('removes a complete draft on Backspace and clears incomplete fields one stage at a time', () => {
    ...
    expect(withoutValue).toMatchObject({
      action: 'update',
      draft: { operator: null, stage: 'operator' },
    })
    expect(
      backspaceFilterDraft(withoutValue.action === 'update' ? withoutValue.draft : complete),
    ).toMatchObject({
      action: 'update',
