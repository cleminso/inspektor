...
- [x] 1. Establish the `JsonView` contract and failing component tests.
  ...
  - Add type-level coverage that rejects `className`, `style`, arbitrary renderers, mutation callbacks, and editable behavior.
...
- [x] 2. Implement the normalized tree model and stable node identity.
  ...
  - Add focused tests for nested objects, primitive arrays, mixed arrays, empty containers, and data replacement.
...
- [x] 4. Implement accessible tree focus and keyboard behavior.
  ...
  - Add interaction tests for traversal, disclosure, focus recovery, and keyboard boundaries.
...
- [x] 5. Add search highlighting without coupling search controls to the component.
  ...
  - Add tests for keys, strings, numbers, booleans, `null`, multiple terms, and empty search state.
...
- [x] 6. Add bounded rendering and continuation behavior.
  ...
  - Add tests for budget boundaries, repeated continuation, focus continuity, and complete source preservation.
...
- [x] 8. Add executable design-system validation surfaces.
  ...
  - Add executable examples for default and collapsed expansion, nested arrays and objects, empty values, search highlighting, long
    strings, bounded branches, and keyboard focus.
...
- [x] 9. Implement the Jazz-to-JSON presentation adapter.
  ...
  - Add unit tests for every Jazz column type, nested arrays, typed and untyped JSON, null, bytes, timestamps, references, and
    unsupported values.
...
- [x] 10. Integrate read-only `JSON` representation into the row side pane.
  ...
  - Prevent table-grid keyboard commands from handling events while focus is inside the JSON tree.
