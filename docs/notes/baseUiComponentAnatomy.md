...
Each part consumes only the shared facts needed for its job. A Trigger knows whether the popup is open. An Item knows whether it is selected, highlighted, or disabled. An Arrow reads positioning geometry. An Indicator reads selection from its nearest Item or control.
