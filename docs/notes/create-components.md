...
Polar's `resolveProperties.ts` resolves constrained layout props to static Tailwind classes; it is not a Base UI state-to-StyleX adapter. Inspektor uses `createStateStyleProps` to define the state mapping once and provide both callbacks expected by Base UI.
