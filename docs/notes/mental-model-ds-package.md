...
The choice is not based on JavaScript compilation speed alone; both complete that phase quickly, while checked TypeScript declarations dominate an empty-cache build. TypeScript checking was the bottleneck.
...
StyleX remains uncompiled so source and distribution modules retain the identities its compiler requires. The consuming application compiles StyleX and owns final CSS extraction.
